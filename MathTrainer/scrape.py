...
for contest in ("AMC 10", "AMC 12"):
    for year in range(2002, 2019):
        for letter in ("A", "B"):
            page = ("http://artofproblemsolving.com/wiki/index.php?title={}_{}_{}{}_Problems".format(year, contest[:-3],
                                                                                                     contest[-2:],
                                                                                                     letter))
            print(page)
            ...
            solutions = "https://artofproblemsolving.com/wiki/index.php?title={}_AMC_{}{}_Answer_Key".format(year, 10,
                                                                                                             letter)
            ...
            for p in range(1, 26):
                ...
                problem = {"problem": problem, "solution": solutions[p - 1].replace("<ol>", ""), "source": contest,
                           "number": p, "difficulty": difficulty, "subject": subject, "id": id}
...
for p in range(1, 16):
    ...
    problem = {"problem": problem, "solution": solutions[p + 1].replace("<ol>", ""), "source": "AIME", "number": p,
               "difficulty": difficulty, "subject": subject, "id": id}
    all_problems.append(problem)
...
for letter in ("I", "II"):
    for year in range(2001, 2019):
        ...
        solutions = "https://artofproblemsolving.com/wiki/index.php?title={}_{}_{}_Answer_Key".format(year, "AIME",
                                                                                                      letter)
        html = requests.get(solutions, timeout=5)
        sol_content = str(BeautifulSoup(html.content, "html.parser"))
        ...
        for p in range(1, 16):
            ...
            problem = {"problem": problem, "solution": solutions[n - 1].replace("<ol>", ""), "source": "AIME",
                       "number": p, "difficulty": difficulty, "subject": subject, "id": id}
            all_problems.append(problem)
print(all_problems)
with open('problems.txt', 'w') as outfile:
    outfile.write("data = ")
    outfile.write("\n")
    outfile.write(str(all_problems))
